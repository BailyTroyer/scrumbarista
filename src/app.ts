...
import { createStandup, createCheckin, scrumbarista } from "./blocks";
...
// Help
app.command("/scrumbarista", async ({ ack, command, say, context }) => {
  await ack();
  try {
    // show all commands avalable
    await slackHandler.postEphemeral(
      token,
      command.channel_id,
      command.user_id,
      scrumbarista,
...
app.command("/standup", async ({ ack, command, say, context }) => {
  ...
  try {
    ...
    const preExistingStandup = await mongooseHandler.getStandup(
      command.channel_id
    );
    console.log("preexisting standup: ", preExistingStandup);
    await slackHandler.openModal(
      token,
      createStandup(preExistingStandup, command.channel_id),
      command.trigger_id
    );
  } catch (error) {
...
app.command("/checkin", async ({ ack, command, say, context }) => {
  ...
  try {
    ...
    console.log("USER ID: ", command.user_id);

    const preExistingCheckin = await mongooseHandler.getCheckin(
      date,
      command.user_id
    );
    ...
    if (preExistingStandup === null) {
      ...
      //   "user",
      //   null,
      //   "There's no standup in this channel. Feel free to create one with /standup"
      // );
      ...
      await slackHandler.openModal(
        token,
        createCheckin(preExistingStandup, preExistingCheckin),
        command.trigger_id
      );
    }
  } catch (error) {
...
app.view("checkin", async ({ ack, body, context, view, payload }) => {
  ...
  try {
    ...
    const standup = await mongooseHandler.getStandup(
    ...
    if (preExistingCheckin !== null) {
      ...
      const updateStandupMessage = await slackHandler.updateMessage(
        token,
        channelID,
        [
          {
      ...
      const standupMessage = await slackHandler.postMessageAttachmentsCustom(
        token,
        channelID,
        [
          {
        ...
        ((userInfo.user as any).profile as any).real_name,
        ((userInfo.user as any).profile as any).image_192
      );
  ...
  try {
    ...
    if (message.channel_type === "im") {
      ...
      if (preExistingCheckin != null) {
        ...
        const answers = !preExistingCheckin.answer
          ? []
          : preExistingCheckin.answer.split("\n");

        if (answers.length === questions.length) {
          // user already answered everything. Send generic feedback message
          await slackHandler.postMessageText(
            token,
            message.channel,
            "Did you know, that you can share feedback with us and at the same time see what we're working on? Hoot this out: https://feedback.olaph.io/",
            ""
          );
        } else {
